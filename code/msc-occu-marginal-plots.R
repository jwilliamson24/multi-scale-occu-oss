...
##### Plot predicted occupancy for each treatment type - ENES ---------------------
...
##### Plot predicted occupancy for each treatment type - OSS ---------------------
...
# Longitude on psi
  ...
  e.long <- ggplot(lon_psi_preds_e, aes(x = cov_value, y = predicted)) +
        geom_line() +
        geom_ribbon(aes(ymin = LCI, ymax = UCI), alpha = 0.2) +
        ylab(bquote("Predicted "*psi~"")) +
        xlab("Longitude") +
...
# Elevation on psi
  ...
  e.elev <- ggplot(elev_psi_preds_e, aes(x = cov_value, y = predicted)) +
        geom_line() +
        geom_ribbon(aes(ymin = LCI, ymax = UCI), alpha = 0.2) +
        ylab(bquote("Predicted "*psi~"")) +
        xlab("Elevation (m)") +
...
##### Combined Psi Plots - both spp -----------------------------------------------
  ...
  lat_psi_preds_e$covariate <- "lat"
  lon_psi_preds_e$covariate <- "long"  
  elev_psi_preds_e$covariate <- "elev"  
...
# multi species plot
  ...
  p <- ggplot(occu_cov_preds2, aes(x = cov_zsc, y = predicted,
    ...
    ylab(bquote("Predicted "*psi~"")) +
    facet_grid(rows = vars(covariate)) +
    labs(title = "Predicted Occupancy Across Environmental Gradients",
         x = "Z-Scored Covariate Value",
         y = expression("Predicted "*psi)) +
    scale_color_manual(values = c("ENES" = "#F95738", "OSS" = "#00798C")) +
...
# Downed Wood effect on Plot Use
  ...
  #ggsave("figures/e-dw-effect.png", plot = e.dw, dpi = 300)
...
# Downed Wood effect on Plot Use
  ...
  # add species
  DW_psi_preds_o <- DW_psi_preds
  DW_psi_preds_o$species <- "OSS"
...
# stack plots above
  p <- (e.dw | e.temp) / (o.dw | o.temp) +
    plot_annotation(
      title = "Predicted Occupancy Across Species and Conditions"
    )
  
  
# Combine DW both spp
  
  both_dw <- rbind(DW_psi_preds_e, DW_psi_preds_o)
